    ...
    match command {
        ...
        Commands::FindSalt { salt } => {
            let Some(salt) = salt else {
                eprintln!("gaal find-salt — Find the JSONL session file containing a salt token");
                eprintln!();
                eprintln!("Usage: gaal find-salt <salt-token>");
                eprintln!();
                eprintln!("The salt token is generated by `gaal salt` and embedded in the session JSONL.");
                eprintln!("This command scans ~/.claude/projects/ and ~/.codex/ for a matching file.");
                eprintln!();
                eprintln!("Examples:");
                eprintln!("  gaal find-salt gaal_abc123def456");
                return Ok(());
            };
        ...
        Commands::Tag { id, tags, remove } => {
            let Some(id) = id else {
                eprintln!("gaal tag — Apply or remove tags on a session");
                eprintln!();
                eprintln!("Usage: gaal tag <session-id> <tag1> [tag2 ...]");
                eprintln!("       gaal tag ls");
                eprintln!();
                eprintln!("Flags:");
                eprintln!("  --remove    Remove tags instead of adding them");
                eprintln!();
                eprintln!("Examples:");
                eprintln!("  gaal tag abc123 bugfix deployment");
                eprintln!("  gaal tag abc123 --remove old-tag");
                eprintln!("  gaal tag ls");
                return Ok(());
            };
