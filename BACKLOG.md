...
| --- | --- | --- |
| P1 | Fresh-clone install validation | Re-run the new `install.sh` path from a clean clone before release tagging. |
| P1 | Public onboarding examples | Keep the first-run prompts sharp around work patterns, attribution, failed commands, topic search, activity checkout, and safe handoff preview. |
...
| P1 | First-run surface polish | Keep README, `docs/getting-started.md`, and `skill/references/first-run.md` aligned around one first-run path, zero-session recovery, `GAAL_HOME`, and scheduled indexing caveats. |
...
- Confirm `README.md`, `docs/getting-started.md`, and
  `skill/references/first-run.md` describe the same first-run flow.
- Add a zero-session recovery checklist for machines with no traces yet.
- Keep `skill/references/first-run.md` as the single first-use prompt set.
