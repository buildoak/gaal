...
| **`recall` returns exit 1 with data** | `gaal recall "topic"` exits 1 even when sessions exist | Handoffs table is empty (no `gaal create-handoff` runs yet) | Run `gaal create-handoff today` first to populate handoffs, then retry recall |
...
| **Inflated active count in index** | `sessions_by_status.active` is much higher than actual running processes | Sessions with `ended_at IS NULL` classified as "active" even when long-dead | Use `gaal active` for accurate live count. `ls --status active` queries the (possibly stale) archive. |
